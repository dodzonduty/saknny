  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      {tab==="buildings"&&(<>
        ...
        {showBF&&(<div className="bg-white rounded-2xl shadow-soft p-6 border-l-4 border-primary"><h3 className="text-lg font-bold text-on-surface mb-4">{editBId?"Edit Building":"New Building"}</h3><form onSubmit={submitB} className="flex flex-wrap gap-4 items-end"><div className="flex-1 min-w-[200px]"><label className="block text-xs font-bold uppercase tracking-widest text-outline-variant mb-1">Name</label><input required value={bName} onChange={e=>setBName(e.target.value)} className="w-full bg-surface-container-lowest border-2 border-outline-variant rounded-xl px-3 py-2 text-sm outline-none focus:border-primary"/></div><div className="w-32"><label className="block text-xs font-bold uppercase tracking-widest text-outline-variant mb-1">Gender</label><select value={bGender} onChange={e=>setBGender(e.target.value)} className="w-full bg-surface-container-lowest border-2 border-outline-variant rounded-xl px-3 py-2 text-sm outline-none focus:border-primary"><option value="M">Male</option><option value="F">Female</option></select></div><div className="w-40"><label className="block text-xs font-bold uppercase tracking-widest text-outline-variant mb-1">Status</label><select value={bStatus} onChange={e=>setBStatus(e.target.value)} className="w-full bg-surface-container-lowest border-2 border-outline-variant rounded-xl px-3 py-2 text-sm outline-none focus:border-primary"><option value="active">Active</option><option value="maintenance">Maintenance</option><option value="inactive">Inactive</option></select></div><button type="submit" disabled={bSub} className="bg-emerald-500 text-white px-5 py-2.5 rounded-xl font-bold hover:bg-emerald-600 disabled:opacity-50">{bSub?"...":editBId?"Update":"Create"}</button></form></div>)}
        {bLoad?<div className="py-8 flex justify-center"><span className="material-symbols-outlined text-primary text-3xl animate-spin">refresh</span></div>:(<div className="bg-white shadow-soft rounded-2xl overflow-hidden"><table className="w-full text-left"><thead className="bg-surface-container-lowest border-b-2 border-outline-variant/30"><tr><th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-on-surface-variant">ID</th><th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-on-surface-variant">Name</th><th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-on-surface-variant">Gender</th><th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-on-surface-variant">Status</th><th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-on-surface-variant text-right">Actions</th></tr></thead><tbody className="divide-y divide-outline-variant/20">{buildings.map(b=>(<tr key={b.dorm_id} className="hover:bg-surface-container-lowest/50"><td className="px-6 py-4 font-bold text-primary">#{b.dorm_id}</td><td className="px-6 py-4 font-semibold text-on-surface">{b.building_name}</td><td className="px-6 py-4"><span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded ${b.gender_type==="M"?"bg-blue-50 text-blue-800":"bg-pink-50 text-pink-800"}`}>{b.gender_type}</span></td><td className="px-6 py-4"><span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded ${b.status==="active"?"bg-emerald-50 text-emerald-800":"bg-surface-container-high text-outline"}`}>{b.status}</span></td><td className="px-6 py-4 text-right"><button onClick={()=>startEB(b)} className="text-xs font-bold text-primary hover:underline">Edit</button></td></tr>))}</tbody></table></div>)}
      </>)}
      {tab==="rooms"&&(<>
